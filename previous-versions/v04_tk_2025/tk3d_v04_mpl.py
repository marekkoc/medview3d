...
# Ustawienie skali dla systemów wysokiej rozdzielczości
...
class MPL(object):
    def __init__(self,img):
        ...
        if isinstance(img, np.ndarray):
            ...
            self.nii = img
            self.im = img.get_fdata()
            self.hdr = img.header
        else:
            print('Wrong data format-(constructor of the MPL class)')
            return
    ...
    def load_image(self, fname):
        ...
        if '.nii' in fname:
            ...
            self.im = self.nii.get_fdata()
            self.hdr = self.nii.header
        elif fname.endswith('.npy'):
            self.im = np.load(fname)
            self.hdr = None
            self.nii = None
        elif fname.endswith('.mat'):
            self.im = io.loadmat(fname)['im']
            self.hdr = None
            self.nii = None
        else:
