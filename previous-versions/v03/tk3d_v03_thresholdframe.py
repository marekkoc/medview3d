...
class CheckButtonScaleFrame(tk.Frame):
    ...
    def __init__(self, parent, mpl):
        ...
        # Label
        label = tk.Label(master=self, text='Thresholding')
        ...
        # checkboks itself
        self.ch = tk.Checkbutton(master=self.gthf, text="Single Th.", variable=self.var, command=self._onCh1)
        ...
        self.ch2 = tk.Checkbutton(master=self.dthf,text='Double Th.', variable=self.var2, command=self._onCh2)
    ...
    def _onGThreshold(self,val):
        if self.var.get():
            ### ODZNACZYC MIP I mIP !!!
            data = np.where(self.mpl.im_o >= int(val), self.mpl.mx, self.mpl.mn)
